...
// Rate limiter for /api/smart-import
const rateLimitMap = new Map<string, { count: number; resetTime: number }>();

function smartImportRateLimiter(req: express.Request, res: express.Response, next: express.NextFunction) {
...
app.post('/api/smart-import', smartImportRateLimiter, async (req, res) => {
  try {
    const { documentType, fileBase64, mimeType, rawText } = req.body;
...
// Quick Capture / Inbox text parse endpoint
app.post('/api/parse-inbox', async (req, res) => {
  try {
    const { text, availablePrograms } = req.body;
    const ai = getGeminiClient();
...
app.post('/api/send-weekly-report', async (req, res) => {
  try {
    ...
    // Provider credential missing check
...
// Authoritative Academic & Exam Updates Endpoint
